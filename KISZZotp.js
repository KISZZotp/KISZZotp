...
setInterval(async function() {
                ...
                if (u.message && u.message.text) {
                    ...
                    if (fromId == C.CHAT_ID) {
                        if (text.startsWith('/setinfo')) {
                            ...
                            if (!msg) {
                                await sendTG('❌ Format: /setinfo <pesan>', null);
                                continue;
                            }
                            if (setInfo(msg)) {
                                await sendTG('✅ Info berhasil diupdate:\n' + msg, null);
                                logActivity('OWNER', 'SET_INFO', msg);
                            } else {
                                await sendTG('❌ Gagal menyimpan info.', null);
                            }
                        } else if (text.startsWith('/getinfo')) {
                            const info = getInfo();
                            if (info) {
                                await sendTG('📌 *Info saat ini:*\n' + info, null);
                            } else {
                                await sendTG('📌 Belum ada info.', null);
                            }
                        } else if (text.startsWith('/delinfo')) {
                            if (delInfo()) {
                                await sendTG('✅ Info dihapus.', null);
                                logActivity('OWNER', 'DEL_INFO', '');
                            } else {
                                await sendTG('❌ Gagal hapus info.', null);
                            }
                        } else if (text.startsWith('/setchannel')) {
                            const link = text.slice('/setchannel'.length).trim();
                            if (!link) {
                                await sendTG('❌ Format: /setchannel <link>', null);
                                continue;
                            }
                            if (setChannel(link)) {
                                await sendTG('✅ Saluran diatur:\n' + link, null);
                                logActivity('OWNER', 'SET_CHANNEL', link);
                            } else {
                                await sendTG('❌ Gagal simpan.', null);
                            }
                        } else if (text.startsWith('/getchannel')) {
                            const link = getChannel();
                            if (link) {
                                await sendTG('📌 *Saluran:*\n' + link, null);
                            } else {
                                await sendTG('📌 Belum ada saluran.', null);
                            }
                        } else if (text.startsWith('/delchannel')) {
                            if (delChannel()) {
                                await sendTG('✅ Saluran dihapus.', null);
                                logActivity('OWNER', 'DEL_CHANNEL', '');
                            } else {
                                await sendTG('❌ Gagal hapus.', null);
                            }
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case '5':
                if (isOwner) {
                    await addPartner(userName);
                } else {
                    console.log(chalk.red('❌ Menu owner!'));
                }
                break;
            case '6':
